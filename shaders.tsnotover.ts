...
function texelIndex(coord: d.v2i, size: d.v2i): number {
  'use gpu';
  const clamped = std.clamp(coord, d.vec2i(0), std.sub(size, 1));
...
function surfaceSlope(gradient: d.v2f): d.v2f {
  ...
  return gradient.mul((ceiling / steepness));
}
...
export const surfaceKernel = tgpu.computeFn({
  ...
  const left = depthTexelAt(coord.add(d.vec2i(GRADIENT_BACK, 0)), size);
  const right = depthTexelAt(coord.add(d.vec2i(GRADIENT_RADIUS, 0)), size);
  const up = depthTexelAt(coord.add(d.vec2i(0, GRADIENT_BACK)), size);
  const down = depthTexelAt(coord.add(d.vec2i(0, GRADIENT_RADIUS)), size);
  const gradient = surfaceSlope(
    std.div(d.vec2f(gentlerDelta(center - left, right - center), gentlerDelta(center - up, down - center)), d.f32(GRADIENT_RADIUS)),
  );
  ...
  for (const radius of tgpu.unroll(OCCLUSION_RADII)) {
    for (const stepY of tgpu.unroll(RING_OFFSETS)) {
      for (const stepX of tgpu.unroll(RING_OFFSETS)) {
        if (stepX !== 0 || stepY !== 0) {
          const neighbor = depthTexelAt(coord.add(d.vec2i(stepX * radius, stepY * radius)), size);
...
function cameraUvAt(uv: d.v2f): d.v2f {
  ...
  const sourcePixel = std.sub((std.sub(sourceSize, side)).mul(0.5).add(framed.mul(side)), 0.5);
  const clamped = std.clamp(sourcePixel, d.vec2f(0), std.sub(sourceSize, 1));
  const sourceUv = std.div((clamped.add(0.5)), sourceSize);
  return relightLayout.$.params.uvTransform.mul((std.sub(sourceUv, d.vec2f(0.5)))).add(d.vec2f(0.5));
}
...
function shadowFactor(origin: d.v3f, lightDirection: d.v3f, reach: number, jitter: number): number {
  ...
  const trailProbe = std.sub(origin, lightDirection.mul(baselineTravel));
  const receiverRise = std.max(
    origin.z - shadowZ(depthAt(trailProbe.xy.add(d.vec2f(0.5)))) - baselineTravel * lightDirection.z,
    d.f32(0),
  );
  ...
  for (const step of std.range(SHADOW_STEPS)) {
    ...
    const sampleZ = shadowZ(depthAt(probe.xy.add(d.vec2f(0.5))));
...
function bulbExposure(radius: number): number {
  ...
  for (const stepY of tgpu.unroll(RING_OFFSETS)) {
    for (const stepX of tgpu.unroll(RING_OFFSETS)) {
      const probe =
        relightLayout.$.params.lightPosition.add(d.vec2f(stepX, stepY).mul((radius * BULB_SAMPLE_SPREAD)));
      open += std.smoothstep(
        d.f32(0),
...
function bulbSurface(uv: d.v2f, tint: d.v3f, depth: number): d.v4f {
  ...
  const spread = std.length(std.sub(uv, relightLayout.$.params.lightPosition)) / radius;
  ...
  return d.vec4f(hue.mul((BULB_CORE * std.mix(d.f32(BULB_LIMB), d.f32(1), facing))), coverage);
}

function bulbGlow(uv: d.v2f, tint: d.v3f): d.v3f {
  'use gpu';
  const radius = bulbRadius();
  const radii = std.length(std.sub(uv, relightLayout.$.params.lightPosition)) / radius;
  const halo = std.exp(0 - radii / BULB_HALO_SPAN);
  const veil = std.exp(0 - radii / BULB_VEIL_SPAN);
  return tint.mul(((halo * BULB_HALO + veil * BULB_VEIL) * bulbExposure(radius)));
}
...
function tonemap(color: d.v3f): d.v3f {
  ...
  const shoulder = std.div(color, d.vec3f(WHITE_POINT * WHITE_POINT)).add(d.vec3f(1));
  const perChannel = std.div((color.mul(shoulder)), (color.add(d.vec3f(1))));
  const bleach = std.pow(std.saturate(mapped), d.f32(HIGHLIGHT_BLEACH));
  return std.saturate(std.mix(color.mul((mapped / luminance)), perChannel, bleach));
}
...
export const relightFragment = tgpu.fragmentFn({
  ...
  const slope = surface.xy.mul((relightLayout.$.params.relief * RELIEF_SCALE));
  const tilt = std.sub(d.vec2f(0), std.div(slope, (1 + std.length(slope) * SLOPE_COMPRESSION)));
  const normal = std.normalize(d.vec3f(tilt, 1));
  if (relightLayout.$.params.mode === RelightMode.NORMALS) {
    return d.vec4f(normal.mul(0.5).add(0.5), 1);
  }

  const centered = std.sub(uv, d.vec2f(0.5));
  const noise = dither(uv);
  const position = d.vec3f(centered, surfaceZ(surface.w));
  const lightPosition = d.vec3f(
    std.sub(relightLayout.$.params.lightPosition, d.vec2f(0.5)),
    relightLayout.$.params.lightZ,
  );
  const toLight = std.sub(lightPosition, position);
  const distance = std.max(std.length(toLight), 0.0001);
  const lightDirection = std.div(toLight, distance);
  ...
  if (relightLayout.$.params.shadow > 0) {
    ...
    const shadowToLight = std.sub(lightPosition, shadowOrigin);
    ...
    const traced = shadowFactor(shadowOrigin, std.div(shadowToLight, shadowDistance), reach, noise);
  ...
  const halfDirection = std.normalize(lightDirection.add(d.vec3f(0, 0, 1)));
  ...
  let lit = albedo.mul(AMBIENT_FILL).mul((relightLayout.$.params.exposure * occlusion));
  lit = lit.add(albedo.mul(tint).mul((lambert * falloff * shadow * relightLayout.$.params.intensity)));
  lit = lit.add(tint.mul((highlight *
      falloff *
      shadow *
      occlusion *
  ...
  lit = std.mix(lit, bulb.xyz.mul(presence), bulb.w * presence);
  lit = lit.add(bulbGlow(uv, tint).mul(presence));
  const display = std.pow(tonemap(lit), d.vec3f(1 / GAMMA));
  return d.vec4f(display.add((noise - 0.5) * DITHER_STEP), 1);
});
