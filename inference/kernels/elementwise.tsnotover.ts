...
export const channelAffineKernel = tgpu.computeFn({
  ...
  const scaled =
    channelAffineLayout.$.src[index].mul(channelAffineLayout.$.scale[params.scaleBase + block]);
  channelAffineLayout.$.dst[index] = maskPaddedChannels(
    scaled.add(channelAffineLayout.$.bias[params.biasBase + block]),
    block,
