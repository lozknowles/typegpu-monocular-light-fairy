...
export const createSpecializedLayerNormKernel = (shape: LayerNormShape) => {
  ...
  return tgpu.computeFn({
    ...
    for (const step of tgpu.unroll(std.range(blocksPerLane))) {
      ...
      let difference = std.sub(layerNormLayout.$.src[pixelBase + block], mean);
      if (maskChannels) {
    ...
    if (!guardPixels || pixel < pixelCount) {
      for (const step of tgpu.unroll(std.range(blocksPerLane))) {
        const block = step * lanes + lid.x;
        const normalized =
          (std.sub(layerNormLayout.$.src[pixelBase + block], mean)).mul(inverseStdDev).mul(layerNormLayout.$.gamma[layerNormLayout.$.params.gammaBase + block]).add(layerNormLayout.$.beta[layerNormLayout.$.params.betaBase + block]);
        if (maskChannels) {
