...
function sampleSourcePixel(pixel: d.v2f, sourceSize: d.v2f): d.v3f {
  'use gpu';
  const maxPixel = std.sub(sourceSize, 1);
  const clamped = std.clamp(pixel, d.vec2f(0), maxPixel);
  const uv = std.div((clamped.add(0.5)), sourceSize);
  const transformedUv = preprocessLayout.$.params.uvTransform.mul((std.sub(uv, d.vec2f(0.5)))).add(d.vec2f(0.5));
...
const depthFramePreprocessKernel = tgpu.computeFn({
  ...
  const cropOrigin = (std.sub(sourceSize, side)).mul(0.5);
  const sourcePixel =
    std.sub(cropOrigin.add((outputPixel.add(0.5)).mul((std.div(d.vec2f(side), d.vec2f(params.outputSize))))), 0.5);
  ...
  for (const tapY of tgpu.unroll(CUBIC_TAPS)) {
    ...
    for (const tapX of tgpu.unroll(CUBIC_TAPS)) {
      ...
      rgb = rgb.add(sampleSourcePixel(d.vec2f(sampleX, sampleY), sourceSize).mul(weight));
      weightSum += weight;
    }
  }

  rgb = std.div(rgb, weightSum);
  const mean = d.vec3f(0.485, 0.456, 0.406);
  const deviation = d.vec3f(0.229, 0.224, 0.225);
  preprocessLayout.$.output[index] = d.vec4f(std.div((std.sub(rgb, mean)), deviation), 0);
});
