...
const inputTransformRow = (x0: d.v4f, x1: d.v4f, x2: d.v4f, x3: d.v4f, x4: d.v4f, x5: d.v4f) => {
  ...
  out[0] = std.sub(x0.mul(4), x2.mul(5)).add(x4);
  out[1] = std.sub(x1.mul(-4), x2.mul(4)).add(x3).add(x4);
  out[2] = std.sub(std.sub(x1.mul(4), x2.mul(4)), x3).add(x4);
  out[3] = std.sub(x1.mul(-2), x2).add(x3.mul(2)).add(x4);
  out[4] = std.sub(std.sub(x1.mul(2), x2), x3.mul(2)).add(x4);
  out[5] = std.sub(x1.mul(4), x3.mul(5)).add(x5);
  return out;
};
...
const outputTransformRow = (x0: d.v4f, x1: d.v4f, x2: d.v4f, x3: d.v4f, x4: d.v4f, x5: d.v4f) => {
  ...
  out[0] = x0.add(x1).add(x2).add(x3).add(x4);
  out[1] = std.sub(std.sub(x1, x2).add(x3.mul(2)), x4.mul(2));
  out[2] = x1.add(x2).add(x3.mul(4)).add(x4.mul(4));
  out[3] = std.sub(std.sub(x1, x2).add(x3.mul(8)), x4.mul(8)).add(x5);
  return out;
};
  ...
  if (pair < pairCount && lane < 4) {
    ...
    for (const step of tgpu.unroll([0, 1, 2, 3])) {
      ...
      if (y < params.height && x < params.width) {
        ...
        transformed = transformed.add(winogradOutputLayout.$.bias[params.biasBase + outputBlock]);
