...
export const maskPaddedChannels = (value: d.v4f, channelBlock: number, logicalChannels: number) => {
  ...
  return std.select(
    d.vec4f(0),
    value,
    std.lt(d.vec4u(baseChannel).add(d.vec4u(0, 1, 2, 3)), d.vec4u(logicalChannels)),
  );
};
...
const erfApprox = (value: d.v4f) => {
  ...
  const t = std.div(d.vec4f(1), (d.vec4f(1).add(absolute.mul(0.3275911))));
  const polynomial =
    ((std.sub(((std.sub(t.mul(1.061405429), 1.453152027)).mul(t).add(1.421413741)).mul(t), 0.284496736)).mul(t).add(0.254829592)).mul(t);
  return std.sign(value).mul((std.sub(d.vec4f(1), polynomial.mul(std.exp(std.sub(d.vec4f(0), value.mul(value)))))));
};

export const geluExact = (value: d.v4f) => {
  'use gpu';
  return value.mul(0.5).mul((d.vec4f(1).add(erfApprox(value.mul(Math.SQRT1_2)))));
};

export const silu = (value: d.v4f) => {
  'use gpu';
  return std.div(value, (d.vec4f(1).add(std.exp(std.sub(d.vec4f(0), value)))));
};
